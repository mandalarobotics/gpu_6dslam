...
#include <fstream>
    ...
    // boost::property_tree::write_xml(fn, pt_, std::locale(), boost::property_tree::xml_writer_make_settings<char>(' ', 1u));
		...
		//caused problems on ubuntu 16.04
...
bool data_model::getGPS (std::string scanId, double &lot, double &lan, double &att)
{
	if (!checkIfExists("Model.Transformations."+scanId+".gps")) return false;
	std::string gps  = pt_.get<std::string>("Model.Transformations."+scanId+".gps");
