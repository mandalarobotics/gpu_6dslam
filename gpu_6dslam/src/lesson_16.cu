		...
		int *d_nearest_neighbour_indexes)
{
	...
	if(index_of_point_second_point_cloud < number_of_points_second_point_cloud)
	{
		...
		float normal_x = d_second_point_cloud[index_of_point_second_point_cloud].normal_x;
		float normal_y = d_second_point_cloud[index_of_point_second_point_cloud].normal_y;
		float normal_z = d_second_point_cloud[index_of_point_second_point_cloud].normal_z;
						...
						if(index_next_bucket >= 0 && index_next_bucket < rgd_params.number_of_buckets)
						{
							...
							for(int l = l_begin; l < l_end; l += iter)
							{
								if(l >= 0 && l < number_of_points_first_point_cloud)
								{
									...
									float nn_normal_x = d_first_point_cloud[hashed_index_of_point].normal_x;
									float nn_normal_y = d_first_point_cloud[hashed_index_of_point].normal_y;
									float nn_normal_z = d_first_point_cloud[hashed_index_of_point].normal_z;
									...
									float dotProduct = normal_x * nn_normal_x +
													   normal_y * nn_normal_y +
													   normal_z * nn_normal_z;
									float angle = acos(dotProduct);//angle = arcos(v1•v2/ |v1||v2|)

									float angled = angle * 180.0f / M_PI;
									if(angled < 0) angled = -angled;
		...
		)
{
	...
	if (ind < number_of_points)
	{
		lidar_pointcloud::PointXYZIRNLRGB  p = d_point_cloud[ind];

		if (p.normal_x * (viewpointX - p.x) + p.normal_y * (viewpointY - p.y) + p.normal_z * (viewpointZ - p.z)  < 0.0)
		{
			d_point_cloud[ind].normal_x = -p.normal_x;
			d_point_cloud[ind].normal_y = -p.normal_y;
			d_point_cloud[ind].normal_z = -p.normal_z;
		}
	}
}
